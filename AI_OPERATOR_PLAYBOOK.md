# AI Operator System — Transfer Spec

Read this entire document as an architecture target. The goal is not a better prompt. The goal is a ChatGPT-centered operator system that survives chat loss, browser crashes, worker timeouts, handoffs, and multiple concurrent model instances.

## What a normal ChatGPT setup will not reliably solve by itself

1. Chat history is not durable authority. Important state must live outside the chat.
2. Having tools is not deterministic routing. Known winning tool paths must be preserved and preferred before guessing or broad search.
3. A successful retry is not prevention. Save both the losing path and the winning path, repair the gate that allowed recurrence, and replay the triggering scenario.
...
6. Multiple chats are not a coordinated fleet. They need shared task authority, presence, messaging, collision rules, idempotency, and handoff semantics.
7. Logs are not enough unless you can reconstruct causally what happened and why the final state is trusted.
8. Source, deployment, runtime, client surface, and durable state can drift apart. Verify parity across the planes that matter.
9. When the model says a capability is not native, treat that as an infrastructure gap when appropriate. Build the missing capability with MCPs, Workers/services, databases, schedulers, repos, or a controller instead of stopping at the limitation.

## Reference architecture

- **M-BRAIN**: durable control plane and continuity layer.
- **MCP Controller**: fleet and infrastructure governor.
- **Correct Path**: deterministic registry of proven winning tool routes.
- **Failure Intelligence**: losing-path + winning-path prevention ledger.
- **Work Units**: canonical task authority.
- **Doors / Fleet Presence**: instance identity, liveness, current work, and active mutation surfaces.
- **M→M**: direct inter-instance message bus with ACK/handoff/yield/rebase/merge semantics.
- **Command Center**: observability UI built from authoritative state and events, not its own competing state machine.

## Authority model

- User: final business authority and veto.
- Operator model: reasoning, planning, execution decisions, tool choice.
...
- MCP Controller: repos, deployments, fleet governance, standards, parity, infrastructure.
...
Do not use memory as the authority for fast-changing live facts. Keep live facts canonical in the owning runtime. Use durable memory/control state for orientation, continuity, decisions, routes, prevention, and work state.
...
- delta/evidence
- requested_action
- response: ACK / HANDOFF / YIELD / REBASE / MERGE / ABORT
...
1. If resuming, read the exact continuation capsule first. Do not start with broad search.
...
10. Persist new winning-path or novel failure/prevention knowledge.
...
Once a non-trivial tool invocation works, preserve the exact recipient, arguments, ordering, prerequisites, expected output, and verification. When that intent appears again, resolve the known route before guessing, broad search, or alternate tool formation.

## Failure Intelligence

Store the losing path and the winning path. If the same failure class recurs, do not call it novel. Reopen the class, identify why the prevention gate failed, repair that gate, and replay from the recurrence point. The target is **novel failures only**.
...
Do not put “wait until later” into the work queue; time belongs to the scheduler. Do not let a worker spend a wake merely summarizing when executable work exists.

## Multi-instance coordination

Every instance needs a stable ID, presence heartbeat, current Work Unit, current action, and declared mutation surfaces.
...
Track at least wall-clock turn time, tool time, calls, failures, last activity, presence/door history, current Work Unit, active mutation surface, and trace ID.
...
A surfaced schema is not proof that the execution path works. Prefer source-exact deployment and independently verified receipts where practical.

## Build order

1. Durable external state store for Work Units, continuation, routes, failures/wins, presence, messages, and events.
2. Gateway/router that resolves intent, authority, continuation, Correct Path, and prevention before tool selection.
...
- Fresh chat resumes the exact next action without broad rediscovery.
